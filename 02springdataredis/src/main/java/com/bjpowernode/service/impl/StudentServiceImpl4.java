 ...
 *  不使用注解的方式
 */
//@Service("studentServiceImpl4")
public class StudentServiceImpl4 implements StudentService {
    ...
    public List<Student> selectStudentsByName(String name) {
        ...
        if (students == null) {
            ...
            synchronized (this){
                //再次从缓存中获取
                students = (List<Student>)ops.get();
                //第二次检测
                if (students == null){
                    System.out.println("selectStudentsByName从数据库中查询，同步代码块");
                    //从数据库中查询
